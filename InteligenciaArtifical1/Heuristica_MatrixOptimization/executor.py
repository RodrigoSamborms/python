...
class MatrixExpressionExecutor:
    ...
    def _compile_code(self):
        ...
        # Atención: `exec` ejecuta código dinámico en tiempo de ejecución.
        # En un entorno académico esto está bien para experimentar, pero
        # en producción hay riesgos de seguridad si las expresiones no
        # son confiables. Validar/limitar entrada antes de `exec`.
    ...
    def execute_serial(self, num_runs: int = 3) -> Tuple[np.ndarray, float, float]:
        ...
        # Ejecutar varias veces para promediar y reducir ruido de tiempo
    ...
    def execute_parallel(self, num_runs: int = 3) -> Tuple[np.ndarray, float, float]:
        ...
        # La versión paralela recibe también `num_workers` como argumento
        for _ in range(num_runs):
