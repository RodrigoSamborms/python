...
class OptimizadorProcesos:
    ...
    def evaluar_estrategia(self, estrategia):
        ...
        estrategia: Lista de booleanos [True, False...] que dice si cada multiplicación usa hilos o no.
        """
        tiempo_ramas = []
        costo_overhead_total = 0
        
        # Evaluamos las multiplicaciones independientes según la estrategia elegida
        for i, nodo in enumerate(self.operaciones_independientes):
            ...
            if usa_hilos:
                # Si usa hilos, se penaliza con el overhead
                tiempo_ramas.append(costo_calculo + self.OVERHEAD)
            else:
                tiempo_ramas.append(costo_calculo)
        
        # Si la estrategia decide paralelizar (al menos una rama usa hilos), 
        # el tiempo de la etapa de multiplicación es el máximo de las dos ramas (corren a la vez).
        # Si ninguna usa hilos, se suman de forma secuencial en un solo núcleo.
        any_paralelo = any(estrategia)
        if any_paralelo and len(tiempo_ramas) >= 2:
            tiempo_multiplicaciones = max(tiempo_ramas)
        else:
            tiempo_multiplicaciones = sum(tiempo_ramas)
            
        # Al final, siempre se le suma el costo secuencial de la suma matricial
