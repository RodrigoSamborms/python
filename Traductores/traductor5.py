...
def cargar_fuente(ruta_archivo):
    """Lee el archivo fuente y devuelve cada linea sin espacios en blanco."""
    with open(ruta_archivo, "r", encoding="utf-8") as archivo:
        return ["".join(linea.split()) for linea in archivo if linea.strip()]
...
if __name__ == "__main__":
    ...
    hubo_error = False
    for numero_linea, fuente in enumerate(fuentes, start=1):
        es_valido, mensaje = analizar(fuente)
        if es_valido:
            print(f"Linea {numero_linea}: {mensaje}")
        else:
            hubo_error = True
            print(f"Linea {numero_linea}: Programa fuente no valido: {mensaje}")
